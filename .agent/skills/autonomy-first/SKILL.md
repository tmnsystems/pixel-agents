---
name: autonomy-first
description: Tina's standing directive on how Antigravity (Gemini) should operate. Antigravity reads this at the start of every session. This skill governs tool selection, proactive execution, and self-improvement behavior.
---
...
3. If a web search can answer a question faster than navigating a browser, use web search. Choose the fastest tool for the job.
4. If multiple approaches exist, choose the one that requires zero effort from Tina.
...
**Fix:** Before starting a new bot instance, always kill ALL existing bun/node processes that might be running the bot. Use `pkill -f "bun"` and wait 3 seconds before starting the new instance.

### 4. Claude Model String Changes

**Problem:** Anthropic retires old model strings. Using outdated strings like `claude-3-5-sonnet-20241022` returns 404 errors.
**Fix:** Before hardcoding a model string, query the available models using `client.models.list()` and use the exact string from the response.
